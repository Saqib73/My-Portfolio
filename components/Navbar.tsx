...
export const Navbar = () => {
  ...
  return (
    <div
      className={`z-50 fixed flex justify-center w-full text-white font-bold transition-transform duration-300 ${
    ...
    >
        ...
        <ul className="flex flex-row p-2 space-x-8">
          {navLinks.map((link, index) => (
            <li key={index}>
              <Link
                href={link.path}
                className="transform hover:text-white/50 transition-all duration-300 ease-out"
              >
                {link.title}
              </Link>
            </li>
      ...
      <div
        onClick={toggleNav}
        className="md:hidden absolute top-5 right-14 border rounded z-50 text-white/70 border-white/70 p-2 cursor-pointer"
      >
